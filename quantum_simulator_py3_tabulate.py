...
def create_state(num_qubits, list_type, lis):
    ...
    if isinstance(list_type, str) == False:
        raise ValueError('Due to requests from participants '\
                        'we changed the input parameters of this function.'\
                                ' Please check the documentation.')
    #if len(lis) < 2**num_qubits:
    if list_type == "indices":
        if len(lis) > 2**num_qubits:
            #raise ValueError('Cannot interpret input of State() creator.'')
            raise ValueError('Too many qubit indices.'\
